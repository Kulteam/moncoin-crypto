...
const MONCoinCrypto = require('../jsbuild/moncoin-crypto')()

// eslint-disable-next-line no-extend-native
Array.prototype.toVectorString = function () {
  const arr = new MONCoinCrypto.VectorString()
...
describe('Cryptography', () => {
  describe('Core', () => {
    it('Generate Random Keys', () => {
      const keys = MONCoinCrypto.generateKeys()
    ...
    it('Check Key - Public Key', () => {
      ...
      const isValid = MONCoinCrypto.checkKey(key)
    ...
    it('Secret Key to Public Key', () => {
      ...
      const generatedKey = MONCoinCrypto.secretKeyToPublicKey(key)

      assert(generatedKey === '7849297236cd7c0d6c69a3c8c179c038d3c1c434735741bb3c8995c3c9d6f2ac')
    })

    it('Generate Key Derivation', () => {
      const derivation = MONCoinCrypto.generateKeyDerivation('3b0cc2b066812e6b9fcc42a797dc3c723a7344b604fd4be0b22e06254ff57f94', '6968a0b8f744ec4b8cea5ec124a1b4bd1626a2e6f31e999f8adbab52c4dfa909')

      assert(derivation === '4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20')
    })

    it('Derive Public Key', () => {
      const publicKey = MONCoinCrypto.derivePublicKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 2, '854a637b2863af9e8e8216eb2382f3d16616b3ac3e53d0976fbd6f8da6c56418')

      assert(publicKey === 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')
    })

    it('Underive Public Key: Ours', () => {
      const publicKey = MONCoinCrypto.underivePublicKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 2, 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')

      assert(publicKey === '854a637b2863af9e8e8216eb2382f3d16616b3ac3e53d0976fbd6f8da6c56418')
    })

    it('Underive Public Key: Not Ours', () => {
      const publicKey = MONCoinCrypto.underivePublicKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 0, 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')

      assert(publicKey !== '854a637b2863af9e8e8216eb2382f3d16616b3ac3e53d0976fbd6f8da6c56418')
    })

    it('Derive Secret Key', () => {
      const secretKey = MONCoinCrypto.deriveSecretKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 2, 'd9d555a892a85f64916cae1a168bd3f7f400b6471c7b12b438b599601298210b')

      assert(secretKey === 'e52ece5717f01843e3accc4df651d669e339c31eb8059145e881faae19ad4a0e')
    })

    it('Generate Key Image', () => {
      const keyImage = MONCoinCrypto.generateKeyImage('bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d', 'e52ece5717f01843e3accc4df651d669e339c31eb8059145e881faae19ad4a0e')

      assert(keyImage === '5997cf23543ce2e05c327297a47f26e710af868344859a6f8d65683d8a2498b0')
    })

    it('Generate Deterministic Subwallet #0', () => {
      const spendKey = MONCoinCrypto.generateDeterministicSubwalletKeys('dd0c02d3202634821b4d9d91b63d919725f5c3e97e803f3512e52fb0dc2aab0c', 0)
    ...
    it('Generate Deterministic Subwallet #1', () => {
      const spendKey = MONCoinCrypto.generateDeterministicSubwalletKeys('dd0c02d3202634821b4d9d91b63d919725f5c3e97e803f3512e52fb0dc2aab0c', 1)
    ...
    it('Generate Deterministic Subwallet #64', () => {
      const spendKey = MONCoinCrypto.generateDeterministicSubwalletKeys('dd0c02d3202634821b4d9d91b63d919725f5c3e97e803f3512e52fb0dc2aab0c', 64)
    ...
    it('Tree Hash', () => {
      ...
      const treeHash = MONCoinCrypto.tree_hash(hashes)
    ...
    it('Tree Branch', () => {
      ...
      const treeBranch = MONCoinCrypto.tree_branch(hashes)
    ...
    it('Generate Ring Signatures', () => {
      ...
      const signatures = MONCoinCrypto.generateRingSignatures(prefixHash, keyImage, publicKeys, privateEphemeral, 3)

      const check = MONCoinCrypto.checkRingSignature(prefixHash, keyImage, publicKeys, signatures)
      assert(check)
    })
    ...
    it('Prepare Ring Signatures', () => {
      ...
      const prep = MONCoinCrypto.prepareRingSignatures(prefixHash, keyImage, publicKeys, 3)

      const signatures = MONCoinCrypto.completeRingSignatures(privateEphemeral, 3, prep.key, prep.signatures)
    ...
    it('Prepare Ring Signatures: Precomputed k', () => {
      ...
      const prep = MONCoinCrypto.prepareRingSignaturesK(prefixHash, keyImage, publicKeys, 3, k)

      const signatures = MONCoinCrypto.completeRingSignatures(privateEphemeral, 3, k, prep.signatures)
    ...
    describe('N/N', () => {
      ...
      describe('Party 1', () => {
        it('Generate Shared Public Spend Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPublicKey([party1.spend.publicKey, party2.spend.publicKey].toVectorString())

          assert(sharedKey === sharedKeys.spend.publicKey)
        })

        it('Generate Shared Private View Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPrivateKey([party1.view.secretKey, party2.view.secretKey].toVectorString())
      ...
      describe('Party 2', () => {
        it('Generate Shared Public Spend Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPublicKey([party2.spend.publicKey, party1.spend.publicKey].toVectorString())

          assert(sharedKey === sharedKeys.spend.publicKey)
        })

        it('Generate Shared Private View Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPrivateKey([party2.view.secretKey, party1.view.secretKey].toVectorString())
      ...
      describe('Transactions', () => {
        it('Restore KeyImage from Partial KeyImages', () => {
          const keyImage1 = MONCoinCrypto.generateKeyImage(tx.publicEphemeral, party1.spend.secretKey)

          const keyImage2 = MONCoinCrypto.generateKeyImage(tx.publicEphemeral, party2.spend.secretKey)

          const keyImage = MONCoinCrypto.restoreKeyImage(tx.publicEphemeral, tx.derivation, tx.input.index, [keyImage1, keyImage2].toVectorString())
        ...
        it('Restore Ring Signatures from Partial Signing Keys', () => {
          ...
          const prep = MONCoinCrypto.prepareRingSignatures(prefixHash, tx.keyImage, publicKeys, 3)

          const sig1 = MONCoinCrypto.generatePartialSigningKey(prep.signatures.get(3), party1.spend.secretKey)

          const sig2 = MONCoinCrypto.generatePartialSigningKey(prep.signatures.get(3), party2.spend.secretKey)

          const sigs = MONCoinCrypto.restoreRingSignatures(tx.derivation, tx.input.index, [sig1, sig2].toVectorString(), 3, prep.key, prep.signatures)

          const success = MONCoinCrypto.checkRingSignature(prefixHash, tx.keyImage, publicKeys, sigs)
          assert(success)
        })
      })
    })
    ...
    describe('N1/N', () => {
      ...
      describe('Party 1', () => {
        it('Generate Multisig Keys', () => {
          const keys = MONCoinCrypto.calculateMultisigPrivateKeys(party1.spend.secretKey, [party2.spend.publicKey, party3.spend.publicKey].toVectorString())
          ...
          assert.deepStrictEqual(tmp, party1.multisig.secretKeys)
        })

        it('Generate Shared Public Spend Key', () => {
          const keys = party1.multisig.publicKeys.concat(party2.multisig.publicKeys, party3.multisig.publicKeys)
          const sharedKey = MONCoinCrypto.calculateSharedPublicKey(keys.toVectorString())

          assert(sharedKey === sharedKeys.spend.publicKey)
        })

        it('Generate Shared Private View Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPrivateKey([party1.view.secretKey, party2.view.secretKey, party3.view.secretKey].toVectorString())
      ...
      describe('Party 2', () => {
        it('Generate Multisig Keys', () => {
          const keys = MONCoinCrypto.calculateMultisigPrivateKeys(party2.spend.secretKey, [party3.spend.publicKey, party1.spend.publicKey].toVectorString())
          ...
          assert.deepStrictEqual(tmp, party2.multisig.secretKeys)
        })
        ...
        it('Generate Shared Private View Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPrivateKey([party2.view.secretKey, party3.view.secretKey, party1.view.secretKey].toVectorString())
      ...
      describe('Party 3', () => {
        it('Generate Multisig Keys', () => {
          const keys = MONCoinCrypto.calculateMultisigPrivateKeys(party3.spend.secretKey, [party1.spend.publicKey, party2.spend.publicKey].toVectorString())
          ...
          assert.deepStrictEqual(tmp, party3.multisig.secretKeys)
        })
        ...
        it('Generate Shared Private View Key', () => {
          const sharedKey = MONCoinCrypto.calculateSharedPrivateKey([party3.view.secretKey, party2.view.secretKey, party1.view.secretKey].toVectorString())
      ...
      describe('Transactions', () => {
        it('Restore KeyImage from Partial KeyImages of Party #1 & Party #2', () => {
          ...
          party1.multisig.secretKeys.forEach(key => {
            const keyImage = MONCoinCrypto.generateKeyImage(tx.publicEphemeral, key)
          ...
          const keyImage = MONCoinCrypto.restoreKeyImage(tx.publicEphemeral, tx.derivation, tx.input.index, partialKeyImages.toVectorString())
        ...
        it('Restore Ring Signatures from Partial Signing Keys of Party #1 & Party #2', () => {
          ...
          party1.multisig.secretKeys.forEach(key => {
            const keyImage = MONCoinCrypto.generatePartialSigningKey(prep.signatures.get(3), key)
          ...
          const sigs = MONCoinCrypto.restoreRingSignatures(tx.derivation, tx.input.index, partialSignatures.toVectorString(), 3, prep.key, prep.signatures)
...
describe('Hash Generation Methods', function () {
  ...
  algos.forEach((algo) => {
    it(algo.name, () => {
      const hash = MONCoinCrypto[algo.func](testdata)
