...
const MONCoinCrypto = require('../')

describe('Cryptography', () => {
  describe('Core', () => {
    it('Generate Random Keys', () => {
      const [err, keys] = MONCoinCrypto.generateKeys()
    ...
    it('Secret Key to Public Key', () => {
      ...
      const [err, generatedKey] = MONCoinCrypto.secretKeyToPublicKey(key)

      assert(!err && generatedKey === '7849297236cd7c0d6c69a3c8c179c038d3c1c434735741bb3c8995c3c9d6f2ac')
    })

    it('Generate Key Derivation', () => {
      const [err, derivation] = MONCoinCrypto.generateKeyDerivation('3b0cc2b066812e6b9fcc42a797dc3c723a7344b604fd4be0b22e06254ff57f94', '6968a0b8f744ec4b8cea5ec124a1b4bd1626a2e6f31e999f8adbab52c4dfa909')

      assert(!err && derivation === '4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20')
    })

    it('Derive Public Key', () => {
      const [err, publicKey] = MONCoinCrypto.derivePublicKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 2, '854a637b2863af9e8e8216eb2382f3d16616b3ac3e53d0976fbd6f8da6c56418')

      assert(!err && publicKey === 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')
    })

    it('Underive Public Key: Ours', () => {
      const [err, publicKey] = MONCoinCrypto.underivePublicKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 2, 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')

      assert(!err && publicKey === '854a637b2863af9e8e8216eb2382f3d16616b3ac3e53d0976fbd6f8da6c56418')
    })

    it('Underive Public Key: Not Ours', () => {
      const [err, publicKey] = MONCoinCrypto.underivePublicKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 0, 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')

      assert(!err && publicKey !== '854a637b2863af9e8e8216eb2382f3d16616b3ac3e53d0976fbd6f8da6c56418')
    })

    it('Derive Secret Key', () => {
      const [err, secretKey] = MONCoinCrypto.deriveSecretKey('4827dbde0c0994c0979e2f9c046825bb4a065b6e35cabc0290ff5216af060c20', 2, 'd9d555a892a85f64916cae1a168bd3f7f400b6471c7b12b438b599601298210b')

      assert(!err && secretKey === 'e52ece5717f01843e3accc4df651d669e339c31eb8059145e881faae19ad4a0e')
    })

    it('Generate Key Image', () => {
      const [err, keyImage] = MONCoinCrypto.generateKeyImage('bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d', 'e52ece5717f01843e3accc4df651d669e339c31eb8059145e881faae19ad4a0e')
    ...
    it('Generate Deterministic Subwallet #0', () => {
      const [err, spendKey] = MONCoinCrypto.generateDeterministicSubwalletKeys('dd0c02d3202634821b4d9d91b63d919725f5c3e97e803f3512e52fb0dc2aab0c', 0)
    ...
    it('Generate Deterministic Subwallet #1', () => {
      const [err, spendKey] = MONCoinCrypto.generateDeterministicSubwalletKeys('dd0c02d3202634821b4d9d91b63d919725f5c3e97e803f3512e52fb0dc2aab0c', 1)
    ...
    it('Generate Deterministic Subwallet #64', () => {
      const [err, spendKey] = MONCoinCrypto.generateDeterministicSubwalletKeys('dd0c02d3202634821b4d9d91b63d919725f5c3e97e803f3512e52fb0dc2aab0c', 64)
    ...
    it('Tree Hash', () => {
      ...
      const [err, treeHash] = MONCoinCrypto.tree_hash([
    ...
    it('Tree Branch', () => {
      ...
      const [err, treeBranch] = MONCoinCrypto.tree_branch([
        'b542df5b6e7f5f05275c98e7345884e2ac726aeeb07e03e44e0389eb86cd05f0',
        '1b606a3f4a07d6489a1bcd07697bd16696b61c8ae982f61a90160f4e52828a7f',
        'c9fae8425d8688dc236bcdbc42fdb42d376c6ec190501aa84b04a4b4cf1ee122',
        '871fcd6823f6a879bb3f33951c8e8e891d4043880b02dfa1bb3be498b50e7578'
    ...
    it('Generate Ring Signatures', () => {
      ...
      const [err, signatures] = MONCoinCrypto.generateRingSignatures(prefixHash, keyImage, publicKeys, privateEphemeral, 3)
      assert(!err)
    ...
    it('Prepare Ring Signatures', () => {
      ...
      const [err, prep] = MONCoinCrypto.prepareRingSignatures(prefixHash, keyImage, publicKeys, 3)
      assert(!err)

      const [err1, signatures] = MONCoinCrypto.completeRingSignatures(privateEphemeral, 3, prep.key, prep.signatures)
      assert(!err1)
    ...
    it('Prepare Ring Signatures: Precomputed k', () => {
      ...
      const [err, prep] = MONCoinCrypto.prepareRingSignatures(prefixHash, keyImage, publicKeys, 3, k)
      assert(!err)

      const [err1, signatures] = MONCoinCrypto.completeRingSignatures(privateEphemeral, 3, k, prep.signatures)
      assert(!err1)
    ...
    describe('N/N', () => {
      ...
      describe('Party 1', () => {
        it('Generate Shared Public Spend Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPublicKey([party1.spend.publicKey, party2.spend.publicKey])
          assert(!err)
        ...
        it('Generate Shared Private View Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPrivateKey([party1.view.secretKey, party2.view.secretKey])
          assert(!err)
      ...
      describe('Party 2', () => {
        it('Generate Shared Public Spend Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPublicKey([party2.spend.publicKey, party1.spend.publicKey])
          assert(!err)
        ...
        it('Generate Shared Private View Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPrivateKey([party2.view.secretKey, party1.view.secretKey])
          assert(!err)
      ...
      describe('Transactions', () => {
        it('Restore KeyImage from Partial KeyImages', () => {
          const [err1, keyImage1] = MONCoinCrypto.generateKeyImage(tx.publicEphemeral, party1.spend.secretKey)
          assert(!err1)

          const [err2, keyImage2] = MONCoinCrypto.generateKeyImage(tx.publicEphemeral, party2.spend.secretKey)
          assert(!err2)

          const [err3, keyImage] = MONCoinCrypto.restoreKeyImage(tx.publicEphemeral, tx.derivation, tx.input.index, [keyImage1, keyImage2])
          assert(!err3)
        ...
        it('Restore Ring Signatures from Partial Signing Keys', () => {
          ...
          const [err1, prep] = MONCoinCrypto.prepareRingSignatures(prefixHash, tx.keyImage, publicKeys, 3)
          assert(!err1)

          const [err2, sig1] = MONCoinCrypto.generatePartialSigningKey(prep.signatures[3], party1.spend.secretKey)
          assert(!err2)

          const [err3, sig2] = MONCoinCrypto.generatePartialSigningKey(prep.signatures[3], party2.spend.secretKey)
          assert(!err3)

          const [err4, sigs] = MONCoinCrypto.restoreRingSignatures(tx.derivation, tx.input.index, [sig1, sig2], 3, prep.key, prep.signatures)
          assert(!err4)
    ...
    describe('N1/N', () => {
      ...
      describe('Party 1', () => {
        it('Generate Multisig Keys', () => {
          const [err, keys] = MONCoinCrypto.calculateMultisigPrivateKeys(party1.spend.secretKey, [party2.spend.publicKey, party3.spend.publicKey])
          assert(!err)
        ...
        it('Generate Shared Public Spend Key', () => {
          ...
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPublicKey(keys)
          assert(!err)
        ...
        it('Generate Shared Private View Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPrivateKey([party1.view.secretKey, party2.view.secretKey, party3.view.secretKey])
          assert(!err)
      ...
      describe('Party 2', () => {
        it('Generate Multisig Keys', () => {
          const [err, keys] = MONCoinCrypto.calculateMultisigPrivateKeys(party2.spend.secretKey, [party3.spend.publicKey, party1.spend.publicKey])
          assert(!err)
        ...
        it('Generate Shared Private View Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPrivateKey([party2.view.secretKey, party3.view.secretKey, party1.view.secretKey])
          assert(!err)
      ...
      describe('Party 3', () => {
        it('Generate Multisig Keys', () => {
          const [err, keys] = MONCoinCrypto.calculateMultisigPrivateKeys(party3.spend.secretKey, [party1.spend.publicKey, party2.spend.publicKey])
          assert(!err)
        ...
        it('Generate Shared Private View Key', () => {
          const [err, sharedKey] = MONCoinCrypto.calculateSharedPrivateKey([party3.view.secretKey, party2.view.secretKey, party1.view.secretKey])
          assert(!err)
      ...
      describe('Transactions', () => {
        it('Restore KeyImage from Partial KeyImages of Party #1 & Party #2', () => {
          ...
          party1.multisig.secretKeys.forEach(key => {
            const [err, keyImage] = MONCoinCrypto.generateKeyImage(tx.publicEphemeral, key)
            assert(!err)
          ...
          const [err, keyImage] = MONCoinCrypto.restoreKeyImage(tx.publicEphemeral, tx.derivation, tx.input.index, partialKeyImages)
          assert(!err)
        ...
        it('Restore Ring Signatures from Partial Signing Keys of Party #1 & Party #2', () => {
          ...
          party1.multisig.secretKeys.forEach(key => {
            const [err, keyImage] = MONCoinCrypto.generatePartialSigningKey(prep.signatures[3], key)
            assert(!err)
          ...
          const [err4, sigs] = MONCoinCrypto.restoreRingSignatures(tx.derivation, tx.input.index, partialSignatures, 3, prep.key, prep.signatures)
          assert(!err4)
...
describe('Hash Generation Methods', () => {
  ...
  algos.forEach((algo) => {
    it(algo.name, () => {
      const [err, hash] = MONCoinCrypto[algo.func](testdata)
